...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/03/22 10:39:39 by dmanoel-          #+#    #+#             */
/*   Updated: 2023/03/22 10:39:42 by dmanoel-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{	
	...
	increments[LON] = (M_PI * 2) / (fdf->map.size[Y] -1);
	increments[LAT] = (M_PI) / (fdf->map.size[X] -1);
	count = 0;
	while (count < fdf->map.points_size)
	{
		point_tmp = &fdf->map.points[count];
